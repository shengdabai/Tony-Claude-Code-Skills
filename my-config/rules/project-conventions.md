---
description: 项目通用约定汇总：批量操作、中文内容处理、下载与网络、MCP 保护、常见陷阱、UI 改动自动验证、GitHub 发布检查、产品上架双审核。
---

# Project Conventions

**本文件的每条约定都对应一次真实事故,动手前先按场景对号入座,别靠临场判断重走老路。**

**Why**:这里的条目不是风格偏好,是复盘产物——批量操作不先跑样本、下载不校验、PDF 并行生成、public repo 不扫内部状态文件,每一条都发生过并造成过返工。规则的价值等于避免重犯的成本。
