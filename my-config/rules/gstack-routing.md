---
description: gstack 与 impeccable 设计 skill 的智能路由表。plan / review / ship / QA / UI 设计类需求出现时，按关键词推荐对应命令。
---

# gstack 智能路由(自动识别 + 菜单推荐)

**命中下方任一关键词时,主动推荐对应命令(一次最多 3 个),并附一句中文说明;没命中就不推荐。**

**Why**:gstack 与 impeccable 加起来几十个命令,靠 Tony 记住命令名等于白装。但反过来每轮都推荐会变成噪音——所以推荐必须由具体关键词触发,而不是由"感觉这个场景可能用得上"触发。
...
- 推荐的充分必要条件 = 用户消息命中本文件列出的具体关键词;没命中关键词一律不推荐,不靠"这个场景可能用得上"发挥
- 一次回复最多 3 条推荐;超过 3 条命中时按「离用户当前动作最近」排序取前 3
- 用户已经在执行某个 gstack 命令时不再推荐同类命令
