---
description: 想法工坊 HTML 深度页生成与自动归档。要把想法 / 链接 / 图片做成深度分析页，或把已有 HTML 纳入本地知识系统时加载。
---

# 想法工坊 · HTML 深度页生成与自动归档

**要做深度 HTML 页时用 `forge` 生成,不要手写一次性 html。**

**Why**:手写的一次性 html 落在随机路径、不进 `pages.json` 索引、没有个性化启发板块,三天后就找不回来。走 `forge` 的产物自动落盘 + 自动归档 + 可全库检索,边际成本几乎为零。
