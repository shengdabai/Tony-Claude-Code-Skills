...
每次会话开始时，**第一件事**是按下面顺序拉历史，在提任何问题之前完成：

### 第 0 步：跨项目历史查 gbrain（如果可用）

如果本机装了 gbrain（用 `command -v gbrain` 判断），先取项目代号 `OPC_TAG="$(basename "$PWD")"`，然后并行跑：

```bash
gbrain list --tag "$OPC_TAG" 2>/dev/null | head -20
gbrain query "$OPC_TAG 当前阶段" 2>/dev/null | head -10
```

把返回的 page slug 列表和摘要作为"过去几个会话沉淀的结论"加入上下文。这一步**只读不写**，brain 没数据或失败都不阻塞，直接进第 1 步。

如果 gbrain 返回了过去某阶段的结论（比如利基定位），且与本地 `opc-doc/` 缺失或冲突，对话中**显式告知用户**：
> "我从 gbrain 找到这个项目过去的[阶段]结论：[摘要]。本地 opc-doc/ 是[空/有/冲突]，你要继续沿用还是从头来？"

### 第 1 步：本地 opc-doc/ 状态恢复
