...
// 使用 playwright(优先全局 homebrew node_modules,回退到 require 默认查找)
let pw;
try {
  pw = require('playwright');
} catch {
  try {
    pw = require('/opt/homebrew/lib/node_modules/playwright');
  } catch {
    pw = require('/usr/local/lib/node_modules/playwright');
  }
}
...
const htmlPath = process.argv[2]
  ...
  : new URL('../templates/result-card.html', import.meta.url).pathname;
const outputPath = process.argv[3]
  ? path.resolve(process.argv[3])
  : new URL('../templates/result-card.png', import.meta.url).pathname;
...
async function screenshot() {
  ...
  // 自动打开图片(可用 NO_OPEN=1 环境变量跳过,避免无人监督时弹预览)
