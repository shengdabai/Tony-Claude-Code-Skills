# Deep Extraction, Anti-Slop, and Response Behavior

This file holds the deep extraction, anti-slop, and example-interpretation sections for the `image-to-code` skill.
The main `SKILL.md` keeps the core directive and image-generation rules (sections 1–21). Load this file when entering the extraction stage, hunting AI-slop tells, or finalizing implementation behavior.
...
After generating the reference image(s), implement the website in a copy-oriented way.
...
9. Is every distinct text style logged with font / weight / size / line-height / letter-spacing / color?
10. Are spacing relationships logged in concrete units (px / rem / `gap-` classes)?
11. Are buttons and components extracted with width / height / padding / radius / state styles?
12. Are colors logged as hex codes plus role labels (background / surface / text / accent)?
...
9. regenerate a section as a fresh standalone image whenever its current image renders text below 14px or buttons below 32px effective size
...
17. enforce strong image usage in hero, showcase, and case-study sections at minimum
...
Inside Codex, if the user wants multiple sections, generate one image per section. The rule of thumb: any time compressing two sections into one would shrink text below 14px, buttons below 32px, or spacing below 8px effective size, split them.
