...
Inside Codex, do not compress more than 2 website sections into a single image. If text would render below 14px effective size, buttons below 32px effective height, or spacing below 8px effective gap, split the sections into separate images instead.
...
- continue the same one-image-per-section pattern until every section has a dedicated image
...
Outside Codex (Claude Code, Cursor, web Claude), this skill still allows up to 3 sections per composite image, as long as each section's text remains at least 14px effective size.
...
- flatter hierarchy when the reference image shows fewer than 3 depth levels
...
- 12 sections → generate 12 section images by default; only collapse two adjacent sections into one image if both are smaller than half a viewport tall
...
- navbar labels
- footer labels if the reference image includes them
...
## 22+. Extraction, anti-slop, response behavior

The deeper directives — Typography / Spacing / Button / Color extraction rules, Design-to-Code Copy Discipline, Anti-Drift, Missing Detail Resolution, Anti-AI-Slop, Typography-First Discipline, Section Rhythm, Density & Spacing, Default Section Packs, Multi-Image Consistency, Clarity Check, Response Behavior, Examples, Final Goal — live in `references/deep-extraction-rules.md` (sections 22–38).

Load `references/deep-extraction-rules.md` when:
- you start the post-image analysis pass on a generated reference
- you are extracting typography / spacing / buttons / colors from any reference image
- you need the anti-AI-slop checklist before declaring a section done
- you are doing the pre-ship Clarity Check
- you are formatting the final response or working from an example brief

**Quick reference — internal TOC of sections 22–38 (full text in `references/deep-extraction-rules.md`):**

- §22 Typography Extraction — font family / weight / size / line-height / letter-spacing / color / alignment / hierarchy per style
- §23 Spacing Extraction — distances, gutters, padding, cadence
- §24 Button / Component Extraction — shape, radius, fill, hierarchy
- §25 Color Extraction — bg / panel / accent / text / border / shadow / gradient
- §26 Design-to-Code Copy Discipline — visually faithful, not “inspired by”
- §27 Anti-Drift Implementation — do not simplify into default templates
- §28 Missing Detail Resolution — preserve language, then regen, then fill
- §29 Anti-AI-Slop — layout / visual / typography / content / density slop bans
- §30 Typography-First Discipline — size contrast, reading order, display moments
- §31 Section Rhythm — vary density, ratio, alignment, scale
- §32 Density & Spacing — even gaps, controlled negative space
- §33 Default Section Packs — 4 / 8 / 12-section presets
- §34 Multi-Image Consistency — brand world, type scale, CTA, icon, tone
- §35 Clarity Check — 21-item pre-ship audit
- §36 Response Behavior — 22-step generation playbook
- §37 Example Interpretations — hero only, 8-section page, 4-section agency
- §38 Final Goal — premium, art-directed, faithful image-to-code translation

Load `references/deep-extraction-rules.md` when extraction quality or anti-drift discipline matters.
Analyze it: identify font family, weight, size, line-height, letter-spacing, color, alignment, and hierarchy level for every distinct text style in the image before coding.

## When to use

Apply this skill when the user wants a website built from one or more reference images (uploaded screenshots, generated design boards, or art-directed reference visuals) reproduced faithfully into running frontend code at full readable scale.

## Scope note

This skill is image-first reproduction. For greenfield design without a reference image pick a taste skill: `design-taste-frontend` (balanced), `minimalist-ui`, `industrial-brutalist-ui`, `gpt-taste`, or `high-end-visual-design`. For redesigning an existing repo use `redesign-existing-projects`. For brand identity boards use `brandkit`. For mobile app screen images use `imagegen-frontend-mobile`. For web design reference images use `imagegen-frontend-web`. For Stitch `DESIGN.md` docs use `stitch-design-taste`.
