...
- IMAGE_GENERATION_EAGERNESS: 10  
  `(1 = minimal screens (3-5), 10 = exhaustive screens plus detail views (15+))`
...
- optionally extra detail views for any screen whose interaction state cannot be inferred from the main screen image
...
Use familiar mobile patterns when the brief targets a mainstream consumer app category (social, finance, productivity, commerce, fitness):
...
For app categories where atmosphere matters (lifestyle, travel, content, premium consumer), introduce subtle to medium-strength texture to create a richer visual atmosphere. Skip texture for clinical, medical, or admin-tool categories.
...
For onboarding, hero, splash, profile-cover, and content-discovery screens, place a full-bleed image behind or beneath text with a darkening overlay so body copy stays at least 4.5:1 contrast.
...
- iconography drawn from the product's domain vocabulary
...
- restrained iconography matching the brand's stroke weight and corner style
...
## 22+. Style, palette, category bias, quality check, examples, final goal

The deeper directives — Style Variation Engine, Color Palette, Non-Genericity, Image System, Fixed Media Frame, Text/Typography/Spacing, Screen-to-Screen Variation, Category-Specific Bias, Regeneration, Quality Check, Response Behavior, Examples, Final Goal — live in `references/style-and-categories.md` (sections 22–38).

Load `references/style-and-categories.md` when:
- you are about to choose a Theme / Typography / Structure / Image-Direction / Texture / Palette / Component / Decorative / Motion combination (start of every new design set)
- you need category-specific bias (fintech, health, productivity, social, commerce, wellness)
- you are doing the pre-ship quality check
- you are formatting the final response or working from an example brief

To avoid repetitive mobile design output, choose a clear visual direction and commit to it — full menu of choices is in `references/style-and-categories.md`.

**Quick reference — internal TOC of sections 22–38 (full text in `references/style-and-categories.md`):**

- §22 Style Variation Engine — Theme / Typography / Structure / Image-Direction / Texture / Palette / Component / Decorative / Motion menus
- §23 Color Palette — keep saturation, accents, palette logic intentional
- §24 Non-Genericity — push past template fintech / wellness / social clichés
- §25 Not Always Simple — clean > minimal; richer is fine if readable
- §26 Image System — when and how images belong in a mobile flow
- §27 Fixed Mobile Media Frame — stable aspect ratios, repeatable modules
- §28 Text Rule — concise headlines, believable labels
- §29 Text Size & Readability — if text feels small, design is not finished
- §30 Typography Rule — strong title/body/label contrast, mobile scale
- §31 Spacing & Density — breathable, touch-friendly, balanced
- §32 Screen-to-Screen Variation — ≥3 differentiating elements per neighbor
- §33 Category-Specific Bias — Fintech / Health / Productivity / Social / Commerce / Wellness
- §34 Regeneration Rule — when to redo a screen
- §35 Quality Check — 27-item pre-ship audit
- §36 Response Behavior — 28-step generation playbook
- §37 Example Interpretations — fitness app, ecommerce app, onboarding flow
- §38 Final Goal — premium, app-native, anti-generic mobile screens

Load `references/style-and-categories.md` at the start of a new design set, when you need category-specific bias, before the pre-ship quality check, or when formatting the final response.


## When to use

Apply this skill when the user asks for premium mobile app screen concepts, iOS/Android UI references, multi-screen app flows, or art-directed mobile design boards as images (not as running code).

## Scope note

This skill outputs mobile app screen images. It does not write running mobile code or implement screens. For implementation from those images use `image-to-code-skill`. For web design reference images use `imagegen-frontend-web`. For brand identity boards use `brandkit`. For greenfield web UI design pick a taste skill: `design-taste-frontend`, `minimalist-ui`, `industrial-brutalist-ui`, `gpt-taste`, or `high-end-visual-design`. For redesigning an existing app use `redesign-existing-projects`.
