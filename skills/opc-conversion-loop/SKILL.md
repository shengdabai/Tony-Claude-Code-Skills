...
如果环境变量 `OPC_AUTO_SYNC=1`，落盘完成后**自动**调用：

```bash
OPC_TAG="$(basename "$PWD")" ~/.claude/scripts/opc-to-gbrain.sh "$OPC_TAG"
```

把本阶段产出同步到 gbrain，以后可跨项目用 `gbrain query "<项目代号> 转化闭环"` 检索回忆。
未设置 `OPC_AUTO_SYNC` 时**不要**自动调，不要询问——用户可手动跑同样命令同步。
