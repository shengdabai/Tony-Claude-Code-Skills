...
### Step 7: Consolidate to Markdown and visual HTML

- Execute canonical prompt 9.
- Treat `NOTE.md` as the canonical content source. Write it beside `SESSION.md` using this structure: objective, sources, plain definition, analogies, accurate mappings, failed mappings, minimum concept set, traps, clarification log, Tony's teach-back, corrections, unresolved questions, review cues, and one most valuable next action.
- Generate a synchronized, single-file `REPORT.html` beside `NOTE.md`. It must preserve every core section and citation from `NOTE.md`; visual polish must not replace or summarize away learning content.
- Make the HTML genuinely visual and usable: clear information hierarchy, table of contents, at least one useful process or relationship visualization when the content supports it, cards/tables/callouts where appropriate, responsive desktop and mobile layouts, readable long-form typography, and print styles.
- Keep the HTML self-contained where practical. Do not rely on a CDN for essential content or styling. Prefer the configured deep-HTML/IdeaForge workflow when available; if it fails or is unavailable, render from `NOTE.md` with a local deterministic toolchain or an existing template instead of silently omitting HTML.
- Preserve source citations and explicitly distinguish facts, inference, and advice in both artifacts.
- Audit both outputs before reporting completion: required-section parity, HTML parse/load, links, desktop and mobile overflow, empty states, and privacy leakage such as names, learner PII, real IPs, secrets, or unintended absolute paths.
- If Tony requests desktop copies, keep the canonical session files and also write clearly named `.md` and `.html` copies to the Desktop.
- Mark the session `complete` only after teach-back contains no critical error and both artifacts pass validation; otherwise mark it `partial` in both files.
- Record artifact paths and validation evidence in `SESSION.md`, then report both saved paths and one most valuable next action.
...
- Markdown and HTML artifact paths plus validation status after consolidation;
...
- Do not ship a visually attractive HTML page that omits core dialogue content, citations, corrections, analogy failures, or unresolved questions.
- Do not let an unavailable HTML generator block Markdown consolidation; use a local fallback and record the failed command or limitation.
...
- `SESSION.md`, `NOTE.md`, and `REPORT.html` agree on status, core sections, citations, and next action;
- the HTML has been checked at desktop and mobile widths, has no unintended horizontal overflow, and contains no core dependency on unavailable external assets.
