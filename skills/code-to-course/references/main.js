...
(function () {
  ...
  $$('.term').forEach(term => {
    ...
    // Bilingual: use data-definition-zh + data-definition-en if available
    const defZh = term.dataset.definitionZh || '';
    const defEn = term.dataset.definitionEn || '';
    if (defZh || defEn) {
      tip.innerHTML =
        (defZh ? '<span class="tip-zh">' + defZh + '</span>' : '') +
        (defEn ? '<span class="tip-en">' + defEn + '</span>' : '');
    } else {
      // Fallback: legacy single data-definition
